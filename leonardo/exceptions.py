...
"""
Exceptions raised by the Leonardo code and the machinery for handling them.
"""
...
class LeonardoException(Exception):
    """Base exception class for distinguishing our own exception classes."""
    pass


class Http302(LeonardoException):
...
class NotAuthorized(LeonardoException):
...
class NotAuthenticated(PermissionDenied, LeonardoException):
...
class AlreadyExists(LeonardoException):
...
class HandledException(LeonardoException):
...
def handle(request, message=None, redirect=None, ignore=False,
    ...
    """Centralized error handling for Leonardo.
    ...
    if issubclass(exc_type, LeonardoException):
        message = exc_value
