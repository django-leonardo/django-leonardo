...
class FolderAdmin(PrimitivePermissionAwareModelAdmin):
    ...
    def _actions_column(self, instance):
        ...
        sync_action = '<a class="ajax-modal" href="{0}">{1}</a>'.format(
            sync_url, _('Scan folder'))
        _actions.append(sync_action)
        create_url = reverse('admin:media-make-folder', args=[instance.id])
        create_action = '<a class="ajax-modal" href="{0}">{1}</a>'.format(
            create_url, _('Create Subfolder'))
    ...
    def delete_files_or_folders(self, request, files_queryset, folders_queryset):
        ...
        if request.POST.get('post'):
            ...
            if n:
                ...
                # methods won't be called and the files won't be deleted from
                # the filesystem.
                folder_ids = set()
    ...
    def _list_folders_to_copy_or_move(self, request, folders):
        for fo in folders:
            ...
            children = list(self._list_folders_to_copy_or_move(
                request, fo.media_folder_children.all()))
            children.extend([self._format_callback(
                f, request.user, self.admin_site, set()) for f in sorted(fo.files)])
            if children:
                yield children
    ...
    def _list_all_to_copy_or_move(self, request, files_queryset, folders_queryset):
        ...
        to_copy_or_move.extend([self._format_callback(
            f, request.user, self.admin_site, set()) for f in sorted(files_queryset)])
    ...
    def _copy_file(self, file_obj, destination, suffix, overwrite):
        if overwrite:
            # Not yet implemented as we have to find a portable (for different
            # storage backends) way to overwrite files
    ...
    def _copy_folder(self, folder, destination, suffix, overwrite):
        ...
        # TODO: Should we also allow not to overwrite the folder if it exists,
        # but just copy into it?
        ...
        folder.insert_at(destination, 'last-child',
                         True)  # We save folder here
    ...
    def _list_folders_to_resize(self, request, folders):
        for fo in folders:
            media_folder_children = list(
                self._list_folders_to_resize(request, fo.children.all()))
            children.extend([self._format_callback(f, request.user, self.admin_site, set(
            )) for f in sorted(fo.files) if isinstance(f, Image)])
            if children:
                yield self._format_callback(fo, request.user, self.admin_site, set())
                yield children
    ...
    def _list_all_to_resize(self, request, files_queryset, folders_queryset):
        ...
        to_resize.extend([self._format_callback(f, request.user, self.admin_site, set(
        )) for f in sorted(files_queryset) if isinstance(f, Image)])
        return to_resize
    ...
    def _resize_image(self, image, form_data):
        ...
        thumbnailer = FilerActionThumbnailer(file=image.file.file, name=image.file.name,
                                             source_storage=image.file.source_storage, thumbnail_storage=image.file.source_storage)
        ...
        if subject_location:
            ...
            (new_x, new_y) = self._new_subject_location(original_width,
                                                        original_height, new_width, new_height, x, y, form_data['crop'])
